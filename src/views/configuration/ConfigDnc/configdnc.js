...
import { SET_MY_CONFIG } from 'store/actions';
...
export default function Dncconfig() {
    ...
    async function getDncConfig() {
        ...
        console.log('Read DNC Config success: ', dnccfg);
        dispatch({ type: SET_MY_CONFIG, myConfig: dnccfg });
        const plugins = dnccfg['plugins'];
        const features = dnccfg['features'];
        setDataConfig({
            Gateway: features['Gateway'],
            DataDownload: plugins['Data Download'],
            Downlink: plugins['Downlink'],
            Subscription: plugins['Subscription'],
            DeviceManagement: features['Device Details'],
            Brixtap: plugins['BrixTap']
        });
    }
    ...
    function handleSave() {
        ...
        updateConfig({ dncconfig: dataconfig });
    }

    function updateDncConfig(mydict) {
        return new Promise(async function (resolve, reject) {
    ...
    async function updateConfig(mydict) {
        try {
            let sresp = await updateDncConfig(mydict);
            toast.success('DNC configuration Updated successfully', {
                position: 'top-right',
                autoClose: 3000,
