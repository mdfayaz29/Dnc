...
export default function ListDmd(props) {
    ...
    Function:
        The getDmdData function is a utility function that retrieves demand 
        data from the server. It makes GET request with authentication headers.

    Definition:
        Processes the response JSON, and formats the data into an array objects
        (myulist).

    Description:
        Each object represents a demand entry with properties such as hardware 
        number (hwsl), board revision (boardrev), firmware version (fwver),
        technology, network, region, action, date of action (doa), and user.

    Return:
        That resolves with an array (myulist) containing formatted demand data.
