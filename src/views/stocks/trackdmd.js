...
export default function TrackDmd(props) {
    ...
    // console.log('ListStock Myprops: ', props.tddata);
    ...
    const thcolumns = [
        ...
        { field: 'fwver', headerName: 'Fw Version', width: 80 },
    ...
    const handleDeleteClick = (id) => () => {
        ...
        Swal.fire({
            ...
            html: `Are you sure, do want to delete the device :
            <pre>${idsToDelete}</pre>
    ...
    function deleteDmd(id) {
        return new Promise(async function (resolve, reject) {
            ...
            let myrow = {};
            myrow['hwsl'] = rows[id - 1]['hwsl'];
            myrow['boardrev'] = rows[id - 1]['boardrev'];
            myrow['fwver'] = rows[id - 1]['fwver'];
            myrow['technology'] = rows[id - 1]['technology'];
            myrow['network'] = rows[id - 1]['network'];
            myrow['region'] = rows[id - 1]['region'];
            myrow['action'] = rows[id - 1]['action'];
            myrow['doa'] = rows[id - 1]['doa'];
            myrow['userid'] = rows[id - 1]['user'];

            let fdict = { mdata: myrow };

            var requestOptions = {
                method: 'DELETE',
                headers: myHeaders,
                body: JSON.stringify(fdict)
            };
    ...
    function getDmdData() {
        return new Promise(async function (resolve, reject) {
            ...
            var url = new URL(DNC_URL + '/tdmd/' + props.tddata.selHwsl);
