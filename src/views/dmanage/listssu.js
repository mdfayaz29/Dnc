...
export default function ListSsu(props) {
    ...
    Function:
        It fetches SSU (Site Service Unit) data by making a GET request to a 
        specified URL with authorization headers. The retrieved data processed
        and transformed into a formatted list before resolving the Promise.

    Definition:
        Asynchronous function getSsuData for fetching SSU data using Fetch API 
        and authorization headers. It returns a Promise.

    Description:
        GET request to the server with authorization headers. It formats the
        received data into a list containing details such as SSU ID (ssuid), 
        batch, type, version (ver), status, client, location, remarks, and 
        arrival date (adate). The formatted list then resolved in the Promise 
        on successful data retrieval or rejected with an error.

    Return:
        fetching SSU data, returning a Promise that resolves with a formatted
        list on success or rejects with an error.
    ...
    function CustomToolbar() {
        return (
            <GridToolbarContainer>
                ...
                {/* <Button style={{}} variant="contained" size="small" color="warning">
                    Add Device
                </Button> */}
            </GridToolbarContainer>
        );
