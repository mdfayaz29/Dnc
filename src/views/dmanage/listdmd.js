...
export default function ListDmd(props) {
    ...
    Function:
        GET request to a specified URL with authorization header. The retrieved
        data is processed and transformed into formatted list before resolving
        the Promise.

    Definition:
        Asynchronous function getDmdData for fetching demand data using Fetch
        API and authorization headers. It returns a Promise.

    Description:
        It formats the received data into a list containing details such as 
        hardware serial number (hwsl), board revision (boardrev), firmware
        version (fwver), technology, network, region, action, date of arrival
        (doa), and user. The formatted list is then resolved in the Promise on
        successful data retrieval or rejected with an error.

    Return:
        Asynchronous function for fetching demand data, returning Promise that
